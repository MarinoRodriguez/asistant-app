...
export default function SessionPage() {
  ...
  const toggle = (personId: number) => {
    ...
    setDirty(true);
    if (typeof window !== 'undefined') window.sessionDirty = true;
  };
  ...
  const save = async () => {
    ...
    setDirty(false);
    if (typeof window !== 'undefined') window.sessionDirty = false;
  };
  ...
  return (
    <div className="d-grid gap-3">
      <div className="d-flex justify-content-between align-items-center">
        ...
        <h1 className="h4 m-0">Sesión {session.date}</h1>
        <div style={{ width: 90 }} />
      </div>
        ...
        <div className="card shadow-sm">
          ...
          <div className="d-flex gap-2">
            ...
            <button className="btn btn-primary" onClick={() => {
              if (window.confirm('Al cerrar la sesión, no podrás editarla. ¿Cerrar ahora?')) close();
